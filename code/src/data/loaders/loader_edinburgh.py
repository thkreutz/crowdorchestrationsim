...
import sys
import cv2
from src.data.loaders.trajdataset import TrajDataset
...
def load_edinburgh_legacy(path, **kwargs):
    ...
    for file in files_list:
        ...
        print(data)
        properties = data[data['index'].astype(str).str.startswith('Properties')]
        data = data[data['index'].astype(str).str.startswith('TRACK')]
...
def load_edinburgh(path, **kwargs):
    ...
    for file in files_list:
        ...
        data = pd.read_csv(file, sep="\n|=", header=None, index_col=None, names=["1", "2"])
        properties = data[data['1'].astype(str).str.startswith('Properties')]
        data = data[data['1'].astype(str).str.startswith('TRACK')]
        ...
        for idx, row in properties.iterrows():
            one_prop = [i.replace(' ',',').replace("[", "").replace("]", "") for i in row["2"].split(";")[0].split(" ")]
            track_length = int(float(one_prop[0]))
            
            one_track = data["2"][idx+1].split(";")
            one_track = one_track[:track_length]
            one_track[0] = one_track[0].replace('[[','[')
            one_track[-1] = one_track[-1].replace(']]',']')
            one_track = np.array([ [x.replace("[", "").replace("]", "") for x in i.replace(' [','[').replace(' ',',').split(",")] for i in one_track]).astype(int)
            
            one_track = np.column_stack( [one_track, np.ones(len(one_track)) * idx//2, track_length * np.ones(len(one_track))])
    ...
    # do not apply homography..
    
    #raw_dataset["centre_x"] = np.array(world_data)[:,0]
    #raw_dataset["centre_y"] = np.array(world_data)[:,1] 

    raw_dataset["centre_x"] = np.array(img_data)[:,0]
    raw_dataset["centre_y"] = np.array(img_data)[:,1] 
